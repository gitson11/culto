...
from src.exporters import export_bulletins_csv, export_bulletins_xlsx, export_louveapp_schedules_csv, export_louveapp_schedules_xlsx
...
BULLETIN_FIELD_SPECS = [
    ("date_text", "Data"), ("dirigente", "Dirigente"), ("preludio_musica", "Preludio"),
    ("preludio_cantor", "Cantor Preludio"), ("preludio_tom", "Tom Preludio"),
    ("musica1", "Musica 1"), ("cantor1", "Cantor 1"), ("tom1", "Tom 1"), ("ref1", "Ref 1"), ("texto1", "Texto 1"),
    ("musica2", "Musica 2"), ("cantor2", "Cantor 2"), ("tom2", "Tom 2"), ("ref2", "Ref 2"), ("texto2", "Texto 2"),
    ("musica3", "Musica 3"), ("cantor3", "Cantor 3"), ("tom3", "Tom 3"), ("ref3", "Ref 3"), ("texto3", "Texto 3"),
    ("oracao_louvor", "Oracao Louvor"), ("ref_louvor", "Ref Louvor"), ("texto_louvor", "Texto Louvor"),
    ("ofertas_ref", "Ofertas Ref"), ("ofertas_texto", "Ofertas Texto"), ("ofertas_oracao", "Ofertas Oracao"),
    ("musica4", "Musica 4"), ("cantor4", "Cantor 4"), ("tom4", "Tom 4"),
    ("musica5", "Musica 5"), ("cantor5", "Cantor 5"), ("tom5", "Tom 5"),
    ("oracao_intercessao", "Oracao Intercessao"), ("pregador", "Pregador"),
    ("musica_pao", "Musica Pao"), ("cantor_pao", "Cantor Pao"), ("tom_pao", "Tom Pao"),
    ("musica_vinho", "Musica Vinho"), ("cantor_vinho", "Cantor Vinho"), ("tom_vinho", "Tom Vinho"),
    ("musica_extra", "Musica Extra"), ("cantor_extra", "Cantor Extra"), ("tom_extra", "Tom Extra"),
    ("musica_final", "Musica Final"), ("cantor_final", "Cantor Final"), ("tom_final", "Tom Final"),
]
LONG_TEXT_FIELDS = {"texto1", "texto2", "texto3", "texto_louvor", "ofertas_texto"}

PERSON_FIELD_SPECS = [
    ("name", "Nome", "entry"),
    ("phone", "WhatsApp / telefone", "entry"),
    ("email", "E-mail", "entry"),
    ("status", "Status", "combo"),
    ("primary_roles", "Funcoes principais", "entry"),
    ("secondary_roles", "Funcoes secundarias", "entry"),
    ("instruments", "Instrumentos", "entry"),
    ("voice", "Voz", "combo"),
    ("availability", "Disponibilidade", "textbox"),
    ("experience_level", "Nivel de experiencia", "combo"),
    ("notes", "Observacoes", "textbox"),
]
STATUS_VALUES = ["ativo", "treinamento", "afastado", "inativo"]
VOICE_VALUES = ["", "soprano", "contralto", "tenor", "baritono", "baixo", "instrumentista"]
EXPERIENCE_VALUES = ["", "iniciante", "intermediario", "experiente", "lider"]
...
class CultoLouveAppManager(ctk.CTk):
    def __init__(self) -> None:
        ...
        self.person_widgets: dict[str, tk.Widget] = {}
        self.title(APP_NAME)
    ...
    def _build_layout(self) -> None:
        ...
        ctk.CTkLabel(title_block, text=f"{APP_SUBTITLE} • {APP_TAGLINE} • {APP_REFERENCE}", font=("Segoe UI", 12), text_color="#E6EEF8").pack(anchor="w", pady=(2, 0))
    ...
    def _build_dashboard_tab(self) -> None:
        ...
        ctk.CTkLabel(tab, text="Painel operacional", font=("Segoe UI", 18, "bold")).grid(row=0, column=0, sticky="w", padx=16, pady=(16, 8))
        ...
        labels = [("bulletins", "Boletins cadastrados"), ("people", "Integrantes ativos"), ("schedules", "Registros de escala importados"), ("last_bulletin", "Ultimo boletim"), ("last_import", "Ultima importacao")]
    ...
    def _build_bulletins_tab(self) -> None:
        ...
        for text, command in (("Novo", self._new_bulletin), ("Salvar", self._save_bulletin), ("Editar", self._edit_bulletin), ("Excluir", self._delete_bulletin), ("Limpar", self._clear_bulletin_form), ("Gerar Boletim", self._generate_bulletin_from_selection), ("Gerar Repertorio", self._generate_repertoire_from_selection)):
        ...
        ctk.CTkLabel(form_panel, text="Use os modelos .docx em templates/ para cada tipo de culto.", anchor="w").grid(row=2, column=0, sticky="ew", padx=12, pady=(0, 6))
        ...
        tree_frame, self.bulletins_tree = self._make_tree(list_panel, ("id", "date", "dirigente", "pregador", "source"), {"id": "ID", "date": "Data", "dirigente": "Dirigente", "pregador": "Pregador", "source": "Origem"}, {"id": 50, "date": 95, "dirigente": 130, "pregador": 130, "source": 90})
    ...
    def _build_people_tab(self) -> None:
        ...
        ctk.CTkLabel(form_panel, text="Perfil do integrante", font=("Segoe UI", 15, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", padx=12, pady=(12, 8))
        for row_index, (field_name, label, field_type) in enumerate(PERSON_FIELD_SPECS, start=1):
            ctk.CTkLabel(form_panel, text=label).grid(row=row_index, column=0, sticky="w", padx=12, pady=4)
            if field_type == "textbox":
                widget = ctk.CTkTextbox(form_panel, height=70, wrap="word")
            elif field_type == "combo":
                values = STATUS_VALUES if field_name == "status" else VOICE_VALUES if field_name == "voice" else EXPERIENCE_VALUES
                widget = ctk.CTkComboBox(form_panel, values=values, state="normal")
            else:
                widget = ctk.CTkEntry(form_panel)
            widget.grid(row=row_index, column=1, sticky="ew", padx=(0, 12), pady=4)
            self.person_widgets[field_name] = widget
        self._set_person_form_defaults()
        actions = ctk.CTkFrame(form_panel, fg_color="transparent")
        actions.grid(row=len(PERSON_FIELD_SPECS) + 1, column=0, columnspan=2, sticky="ew", padx=8, pady=12)
        for text, command in (("Novo", self._clear_person_form), ("Salvar", self._add_person), ("Editar", self._edit_person), ("Inativar", self._inactivate_person)):
            ctk.CTkButton(actions, text=text, width=95, command=command).pack(side="left", padx=4)
        hint = "Separe multiplas funcoes, instrumentos ou dias por virgula. Ex.: Teclado, Vocal | Domingo noite, Quinta."
        ctk.CTkLabel(form_panel, text=hint, wraplength=360, justify="left").grid(row=len(PERSON_FIELD_SPECS) + 2, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, 8))
        list_panel = ctk.CTkFrame(tab)
        list_panel.grid(row=0, column=1, sticky="nsew", padx=(6, 12), pady=12)
        list_panel.grid_columnconfigure(0, weight=1)
        list_panel.grid_rowconfigure(2, weight=1)
        ctk.CTkLabel(list_panel, text="Integrantes do ministerio", font=("Segoe UI", 15, "bold")).grid(row=0, column=0, sticky="w", padx=10, pady=(10, 4))
        search_bar = ctk.CTkFrame(list_panel, fg_color="transparent")
        search_bar.grid(row=1, column=0, sticky="ew", padx=10, pady=6)
        search_bar.grid_columnconfigure(0, weight=1)
        self.person_search_entry = ctk.CTkEntry(search_bar, placeholder_text="Buscar por nome, funcao, instrumento, voz, status ou disponibilidade")
        ...
        tree_frame, self.people_tree = self._make_tree(list_panel, ("id", "name", "status", "roles", "instruments", "availability"), {"id": "ID", "name": "Nome", "status": "Status", "roles": "Funcoes", "instruments": "Instrumentos", "availability": "Disponibilidade"}, {"id": 50, "name": 150, "status": 90, "roles": 170, "instruments": 150, "availability": 220})
        tree_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=(4, 10))
    ...
    def _refresh_template_options(self) -> None:
        values = [path.name for path in list_bulletin_templates()] or ["Nenhum modelo encontrado"]
        for combo_name in ("bulletin_template_combo", "export_template_combo"):
            combo = getattr(self, combo_name, None)
            if combo is not None:
                current = combo.get(); combo.configure(values=values); combo.set(current if current in values else values[0])

    def _selected_template_name(self, combo_name: str = "bulletin_template_combo") -> str | None:
        combo = getattr(self, combo_name, None)
        value = combo.get().strip() if combo else ""
        return None if not value or value == "Nenhum modelo encontrado" else value
    ...
    def _collect_bulletin_form(self) -> WorshipBulletin:
        return WorshipBulletin(**{field: self._widget_value(widget) for field, widget in self.bulletin_widgets.items()}, source="manual")
    ...
    def _new_bulletin(self) -> None:
        self._clear_bulletin_form(); self.tabs.set(TAB_BULLETINS)
    ...
    def _save_bulletin_inner(self) -> str:
        ...
        if not bulletin.date_text: raise ValueError("Informe a data do boletim.")
        self.current_bulletin_id = self.db.insert_bulletin(bulletin)
        self._refresh_bulletins(); self._refresh_dashboard()
        return f"Boletim salvo com ID {self.current_bulletin_id}."

    def _edit_bulletin(self) -> None:
        self._run_action("Editar boletim", self._edit_bulletin_inner)

    def _edit_bulletin_inner(self) -> str:
        if not self.current_bulletin_id: raise ValueError("Selecione um boletim para editar.")
        bulletin = self._collect_bulletin_form()
        if not bulletin.date_text: raise ValueError("Informe a data do boletim.")
        self.db.update_bulletin(self.current_bulletin_id, bulletin); self._refresh_bulletins(); self._refresh_dashboard()
        return "Boletim atualizado."

    def _delete_bulletin(self) -> None:
        if not self.current_bulletin_id:
            messagebox.showwarning("Excluir boletim", "Selecione um boletim para excluir."); return
        if messagebox.askyesno("Confirmar exclusao", "Deseja excluir este boletim?"):
            self._run_action("Excluir boletim", self._delete_bulletin_inner)

    def _delete_bulletin_inner(self) -> str:
        if not self.current_bulletin_id: raise ValueError("Nenhum boletim selecionado.")
        self.db.delete_bulletin(self.current_bulletin_id); self._clear_bulletin_form(); self._refresh_bulletins(); self._refresh_dashboard()
    ...
    def _refresh_bulletins(self, date_search: str = "") -> None:
        for item in self.bulletins_tree.get_children(): self.bulletins_tree.delete(item)
    ...
    def _on_bulletin_selected(self, _event: tk.Event) -> None:
        ...
        if selected:
            ...
            if bulletin: self._load_bulletin_into_form(bulletin)
    ...
    def _generate_bulletin_from_selection_inner(self) -> str:
        ...
        if not self.current_bulletin_id:
            ...
            if not date_text: raise ValueError("Selecione um boletim ou informe uma data.")
    ...
    def _collect_person_form(self) -> WorshipPerson:
        values = {field: self._widget_value(widget) for field, widget in self.person_widgets.items()}
        active = 0 if values.get("status", "ativo").casefold() in {"inativo", "afastado"} else 1
        return WorshipPerson(**values, active=active)

    def _load_person_into_form(self, person: WorshipPerson) -> None:
        self.current_person_id = person.id
        for field, widget in self.person_widgets.items():
            self._set_widget_value(widget, getattr(person, field, ""))

    def _set_person_form_defaults(self) -> None:
        if "status" in self.person_widgets:
            self._set_widget_value(self.person_widgets["status"], "ativo")

    def _clear_person_form(self) -> None:
        self.current_person_id = None
        for widget in self.person_widgets.values(): self._set_widget_value(widget, "")
        self._set_person_form_defaults()

    def _add_person(self) -> None:
        self._run_action("Salvar integrante", self._add_person_inner)

    def _add_person_inner(self) -> str:
        person = self._collect_person_form()
        person_id = self.db.save_person(person)
        self.current_person_id = person_id
        self._refresh_people(); self._refresh_dashboard()
    ...
    def _edit_person_inner(self) -> str:
        if not self.current_person_id: raise ValueError("Selecione um integrante.")
        self.db.update_person(self.current_person_id, self._collect_person_form())
        self._refresh_people(); self._refresh_dashboard()
        return "Integrante atualizado."

    def _inactivate_person(self) -> None:
        if not self.current_person_id:
            messagebox.showwarning("Inativar integrante", "Selecione um integrante."); return
        if messagebox.askyesno("Confirmar", "Deseja inativar este integrante?"):
            self._run_action("Inativar integrante", self._inactivate_person_inner)

    def _inactivate_person_inner(self) -> str:
        if not self.current_person_id: raise ValueError("Selecione um integrante.")
        self.db.inactivate_person(self.current_person_id); self._refresh_people(); self._refresh_dashboard(); self._clear_person_form()
        return "Integrante inativado."

    def _refresh_people(self) -> None:
        if not hasattr(self, "people_tree"): return
        ...
        for person in self.db.list_people(search):
            roles = person.primary_roles or person.secondary_roles
            self.people_tree.insert("", "end", iid=str(person.id), values=(person.id, person.name, person.status or ("ativo" if person.active else "inativo"), roles, person.instruments, person.availability))

    def _on_person_selected(self, _event: tk.Event) -> None:
        selected = self.people_tree.selection()
        if selected:
            person = self.db.get_person(int(selected[0]))
            if person: self._load_person_into_form(person)

    def _start_louveapp_import(self) -> None:
        clear_existing = bool(self.clear_louveapp_var.get())
        if clear_existing and not messagebox.askyesno("Limpar importacao", "Deseja apagar as escalas LouveApp ja importadas antes de salvar a nova importacao?"): return
        self.louveapp_import_button.configure(state="disabled"); self.louveapp_status_label.configure(text="Iniciando importacao...")
        def worker() -> None:
            def progress(message: str) -> None: self.worker_queue.put(("louveapp_status", message))
            self.worker_queue.put(("louveapp_done", import_louveapp_schedules(progress=progress, clear_existing=clear_existing)))
    ...
    def _poll_worker_queue(self) -> None:
        try:
            while True:
                ...
                if event_name == "louveapp_status": self.louveapp_status_label.configure(text=str(payload))
                elif event_name == "louveapp_done": self._finish_louveapp_import(payload)
        except queue.Empty: pass
        self.after(300, self._poll_worker_queue)

    def _finish_louveapp_import(self, result: ImportResult) -> None:
        self.louveapp_import_button.configure(state="normal"); self.louveapp_status_label.configure(text=result.message)
        self._refresh_schedules(); self._refresh_dashboard()
        if result.status == "success": messagebox.showinfo("Importar LouveApp", result.message)
        elif result.status == "warning": messagebox.showwarning("Importar LouveApp", result.message)
        else: messagebox.showerror("Importar LouveApp", result.message)

    def _refresh_schedules(self) -> None:
        if not hasattr(self, "schedules_tree"): return
        search = self.schedule_search_entry.get() if hasattr(self, "schedule_search_entry") else ""
        for item in self.schedules_tree.get_children(): self.schedules_tree.delete(item)
    ...
    def _select_legacy_file(self) -> None:
        ...
        if file_path: self.legacy_path_var.set(file_path)
    ...
    def _append_legacy_summary(self, text: str) -> None:
        self.legacy_summary.delete("1.0", "end"); self.legacy_summary.insert("1.0", text)
    ...
    def _inspect_legacy_file_inner(self) -> str:
        self._append_legacy_summary(json.dumps(inspect_legacy_workbook(self._legacy_path_or_none()), ensure_ascii=False, indent=2)); return "Arquivo inspecionado."
    ...
    def _import_legacy_bulletins_inner(self) -> str:
        result = import_legacy_bulletins(self._legacy_path_or_none(), self.db); self._append_legacy_summary(result.message); self._refresh_bulletins(); self._refresh_dashboard()
        if result.status != "success": raise RuntimeError(result.message)
    ...
    def _import_legacy_people_inner(self) -> str:
        result = import_legacy_people(self._legacy_path_or_none(), self.db); self._append_legacy_summary(result.message); self._refresh_people(); self._refresh_dashboard()
        if result.status != "success": raise RuntimeError(result.message)
        return result.message

    def _export_bulletins_xlsx(self) -> None: self._run_action("Exportar boletins Excel", lambda: f"Arquivo gerado: {export_bulletins_xlsx()}")
    def _export_bulletins_csv(self) -> None: self._run_action("Exportar boletins CSV", lambda: f"Arquivo gerado: {export_bulletins_csv()}")
    def _export_schedules_xlsx(self) -> None: self._run_action("Exportar escalas Excel", lambda: f"Arquivo gerado: {export_louveapp_schedules_xlsx()}")
    def _export_schedules_csv(self) -> None: self._run_action("Exportar escalas CSV", lambda: f"Arquivo gerado: {export_louveapp_schedules_csv()}")

    def _generate_bulletin_by_date_input(self) -> None: self._run_action("Gerar boletim por data", self._generate_bulletin_by_date_input_inner)
    def _generate_bulletin_by_date_input_inner(self) -> str:
        date_text = self.doc_date_entry.get().strip()
        if not date_text: raise ValueError("Informe a data.")
        return f"Boletim gerado: {generate_bulletin_by_date(date_text, template_name=self._selected_template_name('export_template_combo'))}"
    def _generate_repertoire_by_date_input(self) -> None: self._run_action("Gerar repertorio por data", self._generate_repertoire_by_date_input_inner)
    ...
    def _refresh_logs(self) -> None:
        if not hasattr(self, "logs_text"): return
        self.logs_text.delete("1.0", "end")
        if not LOG_PATH.exists(): self.logs_text.insert("1.0", "Nenhum log gerado ainda."); return
    ...
    def _open_folder(self, path: Path) -> None:
        ...
        try:
            ...
            logger.exception("Falha ao abrir pasta"); messagebox.showerror("Abrir pasta", f"Nao foi possivel abrir a pasta: {exc}")

    def _run_action(self, title: str, action: Callable[[], str], show_success: bool = True) -> None:
        try:
            message = action(); self._refresh_logs()
            if show_success: messagebox.showinfo(title, message)
        except Exception as exc:
            logger.exception("Erro na acao: %s", title); self._refresh_logs(); messagebox.showerror(title, str(exc))
